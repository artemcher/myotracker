import torch
import torch.nn as nn
import torch.nn.functional as F

from myotracker.network.model_utils import sample_features4d
from myotracker.network.embeddings import get_2d_embedding
from myotracker.network.blocks import ConvEncoder, AttnBlock, CorrBlock
from myotracker.network.myotracker import TrackFormer

torch.manual_seed(0)

class MyoTrackerIter(nn.Module):

    def __init__(self, input_shape=(64, 1, 256, 256), iterations=6):
        super(MyoTrackerIter, self).__init__()
        self.iterations = iterations
        self.stride = 4
        self.latent_dim = 64

        self.encoder = ConvEncoder(output_dim=self.latent_dim)
        self.model_res = input_shape[-2:]
        
        self.input_dim = 196 # latent (64) + LRR (100) + coord_embedding (32)
        self.updateformer = TrackFormer(
            depth=4,
            input_dim=self.input_dim,
            hidden_size=64,
            num_heads=4,
            output_dim=2,
            mlp_ratio=1.0,
        )

        self.norm = nn.GroupNorm(1, self.latent_dim)
        self.track_feat_updater = nn.Sequential(
            nn.Linear(self.latent_dim, self.latent_dim),
            nn.GELU(),
        )

    def get_track_feat(self, feature_maps, queried_coords):
        sample_track_feats = sample_features4d(feature_maps[:,0], queried_coords)
        return torch.unsqueeze(sample_track_feats, 1)

    def forward(self, frames, queries):
        B, T, C, H, W = frames.shape
        B, N, __ = queries.shape # [B, N, 2], (y,x) - order
        device = queries.device

        queried_coords = queries / self.stride
        feature_maps = self.encoder(frames.reshape(-1, C, H, W)).reshape(
            B, -1, self.latent_dim, H // self.stride, W // self.stride
        )
        
        # compute track features
        track_features = self.get_track_features(feature_maps, queried_coords).repeat(1, T, 1, 1)

        # get correlation maps, sample correlation features around each point
        corr_block = CorrBlock(feature_maps, num_levels=4, radius=2)
        corr_block.corr(track_features)
        coords_init = queried_coords.reshape(B, 1, N, 2).expand(B, T, N, 2) # [B, T, N, 2]

        corr_block = CorrBlock(feature_maps, num_levels=4, radius=2)

        # iterative refinement: diverges from regular MyoTracker back to CoTracker-like flow
        coord_predictions = []
        for __ in range(self.iterations):
            coords = coords.detach()  # [B, T, N, 2]
            corr_block.corr(track_feat)

            # sample correlation features around each point
            fcorrs = corr_block.sample(coords)  # [B, N, T, LRR], LRR = (2*radius+1)*levels

            # get coords. embeddings
            flows = (coords - coords[:, 0:1]).permute(0, 2, 1, 3).reshape(B * N, T, 2)
            flow_emb = get_2d_embedding(flows, 16, cat_coords=False)  # [N, T, E]

            track_feat_ = track_feat.permute(0, 2, 1, 3).reshape(B * N, T, self.latent_dim) # [B*N, T, C]
            transformer_input = torch.cat([flow_emb, track_feat_, fcorrs], dim=2)
            x = transformer_input.reshape(B, N, T, self.input_dim)  # [B, N, T, D]
        
            # get updates for tracks (first 2 channels) and track features (remaining channels)
            delta = self.updateformer(x)

            # handle coord. update
            delta_coords = delta[..., :2].permute(0, 2, 1, 3)
            coords = coords + delta_coords
            coord_predictions.append(coords * self.stride)

            # handle track features update
            delta_feats_ = delta[..., 2:].reshape(B*N*T, self.latent_dim)
            track_feat_ = track_feat.permute(0, 2, 1, 3).reshape(B*N*T, self.latent_dim)
            track_feat_ = self.track_feat_updater(self.norm(delta_feats_)) + track_feat_
            track_feat = track_feat_.reshape(B, N, T, self.latent_dim).permute(
                0, 2, 1, 3
            )  # [B*N*T, C] -> [B, N, T, C]
        return coord_predictions # list with predictions at each iteration